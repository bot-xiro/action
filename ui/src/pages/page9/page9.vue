<!-- 详情页副本 page9 : 渲染共享详情组件, nextPage 指向轮换环的下一站.
     固件同名页 navTo 只替换不叠加, 多个副本维持返回栈 (page->page2->...->page12->page). -->
<template>
  <DetailPage ref="d" :next-page="'page10'" />
</template>
  ...
  methods: {
    ...
    forward: function (hook) {
      ...
      if (d && typeof d[hook] === 'function') {
        try { d[hook]() } catch (e) { console.log('[page9] forward ' + hook + ' error: ' + (e && e.message ? e.message : e)) }
      }
    },
