...
// 默认 zpos：视频(2) > UI(0) → 视频直接盖在 UI 之上，弹幕/控制栏被遮挡；
// 需求"视频在页面最底层"：把 UI primary 提到 zpos=3（高于视频 2）。
// zpos 属性实测可写且 weston 重绘(getcompositor)后不重置，播放前设置一次即可。
static void raiseUiAboveVideoLayer()
{
    int rc = ::system("modetest -M rockchip -w 54:zpos:3 >/dev/null 2>&1");
    if (rc != 0) {
        // 部分固件 modetest 不在 PATH，尝试绝对路径
        rc = ::system("/usr/bin/modetest -M rockchip -w 54:zpos:3 >/dev/null 2>&1");
    }
    syslog(LOG_ERR, "[gstplayer] raise UI primary zpos -> 3 (over video z=2): ret=%d", rc);
}
#endif
...
bool GstPlayer::buildPipeline(const std::string& uri, bool audio, const std::string& rect, const std::string& fill)
{
    ...
    // 视频沉到页面最底层：提升 UI primary plane zpos 到 3（高于视频 plane 的 2），
    // 使弹幕/控制栏浮在视频之上（每次 open 调用一次，天然幂等）
    raiseUiAboveVideoLayer();
